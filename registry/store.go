...
// a clock the host cannot see: somebody closed it ([Room.ClosedAt]), or nobody
// republished it for this long and [Store.limpiar] swept it. Everything else —
// the tunnel, the NAT hole, the engine, the host's whole machine — is
// disposable, and coming back from any of it is the product working.
//
// A live room never gets near this: it republishes hourly, which is what pushes
...
type Room struct {
	...
	// PinUntil is when this entry stops belonging to anybody. See [RoomTTL].
...
func (s *Store) insert(id domain.InviteID, hostKey ed25519.PublicKey, card, sig []byte, red string) bool {
	...
	s.rooms[id.Raw()] = &Room{
		...
		RendezvousName: red,
		PinUntil:       s.currentTime().Add(RoomTTL),
	}
	return true
}
...
func (s *Store) publishUnderLock(id domain.InviteID, hostKey ed25519.PublicKey, card, sig []byte) error {
	...
	r.PinUntil = s.currentTime().Add(RoomTTL)
...
// ends nothing: only this, and the sweep at [RoomTTL].
//
...
//
// Its host closed it, or nobody republished it for [RoomTTL] and the sweep took
