...
type Release struct {
	...
	// del siguiente intento. Ver [timing.ReleaseCheckAfterFailure].
	falló bool
}

func NewRelease() *Release {
	return &Release{preguntar: selfupdate.Latest, cadencia: timing.ReleaseCheck}
}
...
func (r *Release) esperaActual() time.Duration {
	if r.falló {
		return timing.ReleaseCheckAfterFailure
	}
	return r.cadencia
}
...
// se vuelve a intentar, ver [timing.ReleaseCheckAfterFailure].
func (r *Release) refrescar() {
	ctx, cancel := context.WithTimeout(context.Background(), timing.ReleaseTimeout)
	defer cancel()
