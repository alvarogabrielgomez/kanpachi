...
- Cut the release notes down to what somebody deciding which file to download needs. The body repeated the whole product's story on every publication, so the two lines that change between versions sat under a page and a half that never does (PENDING)
