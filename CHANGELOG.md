...
- Come back to a room in seconds and with the SAME address when the tunnel dies under you, instead of asking the host for a new credential once a minute and burning a fresh address per lap: a guest now rebuilds its tunnel with the credential it already holds, so the game server never notices, and the full exchange through the lobby remains what runs when the host really did forget everybody (pending-commit-link)
