...
- Go back from choosing a server, typing its password and changing your name. All three were entered and only left by completing them
- Show the button working while it works, on those same three screens: saving went quiet for as long as the service took, and a disabled button looks exactly like one that did nothing
- Stop printing raw internal errors where the screen explains what is wrong with what you typed. Choosing a server could answer `DaemonUnreachable(timedOut): el daemon no contestó a own_seed en 10 s`, which reads as if the name were the problem
