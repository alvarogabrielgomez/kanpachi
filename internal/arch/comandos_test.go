...
//
// `daemon/cmd/kanpachi` lleva DOS listas de comandos: `comandos`, que es la
// tabla que despacha, y `grupos`, que es de dónde `kanpachi help` saca lo que
...
func TestTodoComandoDelCLIApareceEnLaAyuda(t *testing.T) {
	const archivo = "../../daemon/cmd/kanpachi/comandos.go"
	...
	if len(enLaTabla) == 0 {
		t.Fatalf("no se encontró la tabla `comandos` en %s: la forma cambió y este test no vigila nada", archivo)
	}
	enLaAyuda := nombresDeLosGrupos(t, fuente)
	if len(enLaAyuda) == 0 {
		t.Fatalf("no se encontró la lista `grupos` en %s: la forma cambió y este test no vigila nada", archivo)
	}

	for _, nombre := range enLaTabla {
		if !enLaAyuda[nombre] {
			t.Errorf("el comando %q existe y `kanpachi help` no lo nombra: "+
				"agregalo a `grupos`, o nadie va a enterarse de que está", nombre)
		}
	}
	...
	for nombre := range enLaAyuda {
		if !tabla[nombre] {
			t.Errorf("`grupos` nombra %q, que no está en la tabla de comandos: "+
				"la ayuda imprimiría una línea vacía", nombre)
		}
	}
}

// clavesDeLaTabla saca los nombres de comando de `comandos = map[string]comando{…}`.
//
...
func clavesDeLaTabla(t *testing.T, fuente *ast.File) []string {
	...
	ast.Inspect(fuente, func(n ast.Node) bool {
		...
		if id, ok := asig.Lhs[0].(*ast.Ident); !ok || id.Name != "comandos" {
			return true
		}
...
func nombresDeLosGrupos(t *testing.T, fuente *ast.File) map[string]bool {
	...
	for _, decl := range fuente.Decls {
		...
		for _, spec := range gen.Specs {
			...
			if !ok || len(valor.Names) != 1 || valor.Names[0].Name != "grupos" {
				continue
			}
