...
void main() {
  group('toda clave del daemon tiene texto', () {
    test('cada alerta trae título y cuerpo', () {
      for (final AlertKind kind in AlertKind.values) {
        ...
        expect(
          ...
          isFalse,
          reason:
              'la alerta ${kind.wire} no tiene texto. El módulo de '
        ...
        expect(
          ...
          isNotEmpty,
          reason:
              'la alerta ${kind.wire} dice qué pasó y no qué significa. '
    ...
    test('cada código de error trae cuerpo', () {
      for (final FailureCode code in FailureCode.values) {
        expect(
          ...
          isNotEmpty,
          reason:
              'el código ${code.wire} no tiene texto, así que la operación '
    ...
    test(
      'cada motivo de salida trae texto, salvo el de salir por tu cuenta',
      () {
        for (final ExitReason reason in ExitReason.values) {
          ...
          if (reason == ExitReason.user) {
            ...
            expect(
              m.isEmpty,
              isTrue,
              reason: 'salir por tu cuenta no se explica',
            );
            continue;
          }
          expect(
            ...
            isFalse,
            reason:
                'volver a la portada por ${reason.wire} no dice nada, así '
  ...
  group('las reglas ajenas del firewall', () {
    test('cada clase trae título y cuerpo', () {
      for (final RuleClass kind in RuleClass.values) {
        ...
        expect(
          ...
          isFalse,
          reason:
              'la clase ${kind.wire} no tiene texto, así que el hallazgo '
        ...
        expect(
          ...
          greaterThan(20),
          reason:
              'el cuerpo de ${kind.wire} es demasiado corto para explicar '
    ...
    test('la de control remoto nombra lo que se entrega', () {
      ...
      for (final String palabra in <String>[
        'teclado',
        'pantalla',
        'archivos',
      ]) {
        expect(
          ...
          isTrue,
          reason:
              'el aviso bloqueante no dice "$palabra": sin nombrar lo que '
      ...
      for (final AppMessage m in todos) {
        ...
        for (final String clave in claves) {
          expect(
            ...
            isFalse,
            reason:
                'el texto "$texto" lleva la clave $clave dentro. La regla '
  ...
  group('lo que llega crudo del cable', () {
    test(
      'una clave desconocida sale como el mensaje de reserva, no en blanco',
      () {
        final AppMessage m = AppMessages.alertFromWire(
          'inventada_en_el_futuro',
        );
    ...
    test('una clave conocida se resuelve igual que por enum', () {
      for (final AlertKind kind in AlertKind.values) {
        expect(
          ...
          reason:
              'la clave ${kind.wire} se resuelve distinto según por dónde '
              'entre',
        );
      }
    });
    ...
    test('un código de error desconocido no se traga', () {
      expect(
        AppMessages.failureFromWire('codigo_del_futuro'),
  ...
  group('el detalle del daemon', () {
    test('acompaña al mensaje sin sustituir el cuerpo', () {
      const String detalle =
          'el router publica el puerto 25565 hacia 192.168.1.7';
      final AppMessage conDetalle = AppMessages.alert(
      ...
      expect(
        ...
        reason:
            'el copy lo decide la clave; el detalle del daemon acompaña. '
    ...
    test('un detalle vacío no deja el campo puesto', () {
      expect(
        AppMessages.alert(AlertKind.firewallOff, detail: '').detail,
        isNull,
      );
      expect(
        AppMessages.alert(AlertKind.firewallOff, detail: null).detail,
        isNull,
      );
    });
  });
