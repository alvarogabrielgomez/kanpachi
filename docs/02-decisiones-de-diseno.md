...
**Por qué esa frontera es toda la decisión.** En el sidecar la intención no es ambigua: ese contenedor declaró un juego, el servidor vive dentro del mismo espacio de red y el compose entero existe para servir esa sala. No hay red de casa que proteger y nadie ató nada para esconderlo. En una instalación nativa es al revés: atar un servicio a una dirección es lo que hace alguien para que NO lo alcancen desde otro sitio, y reescribir eso por su cuenta rompería una decisión ajena. Ahí esto no llega a existir, porque `redirectFor` devuelve `nil` y el puerto de desvío no se cablea. La bandera es explícita, `KANPACHI_CONTAINER=1` puesta por el entrypoint de la imagen, y no adivinada por `/.dockerenv` ni por cgroups: de ella cuelga que Kanpachi reescriba destinos, así que tiene que leerse en un diff.
...
**No es invisible y no sobrevive.** La sala y el `kanpachi status` dicen hacia dónde se está desviando, y ponerlo o quitarlo se anuncia en el acto: el latido compara la salud Y el desvío, porque con desvío puesto la sala sí alcanza el juego y sin él no, y comparando solo la salud ese cambio esperaba hasta dos minutos al anuncio periódico. La tabla se borra al salir de la sala, al quitar el juego y en cuanto la medición dice que ya no hace falta, y no sobrevive a un reinicio, igual que la compuerta. Como no persiste, no necesita libro.
