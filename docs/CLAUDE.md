...
Está en `docs/`. Son la fuente de verdad del diseño, no notas sueltas. Los siete primeros describen el producto, el octavo es el plan de trabajo y envejece rápido a propósito, y el noveno es una tabla de consulta.
...
| `09-relojes.md` | Cada bucle periódico: de quién es, qué toca y si pide el candado de la sesión | Colgar trabajo de un latido, o preguntarte cada cuánto se refresca algo |

Fuera de `docs/` hay uno más: **`CHANGELOG.md`** en la raíz, que cuenta qué cambió en cada versión para quien usa Kanpachi. Los nueve de arriba explican el diseño; ese cuenta la historia. Cómo se mantiene está abajo.

### La documentación pública, en `docs/public/`

Aparte de los nueve de diseño está **la documentación pública, en inglés**. Su índice es `docs/public/README.md`. Los nueve de arriba son para quien cambia el código; estos son para quien usa Kanpachi, y **no se escriben en castellano**, por lo mismo que `kanpachi-protection.md`: se enlazan desde fuera del repositorio.
...
| Un reloj: uno nuevo, uno que cambia de dueño, o trabajo que cuelgas de un latido | `09` |
...
- Los nueve documentos tienen que ser consistentes entre sí. Mismo código de ejemplo, mismos puertos, mismo nombre de adaptador y de servicio.
...
- **Dos filtros, y hay que pasar los dos: stop-slop para la prosa y [Diátaxis](https://diataxis.fr/) para la estructura.** Stop-slop tiene skill instalado: invócalo antes de entregar prosa. Quita adverbios, pasiva sin actor, arranques con Wh- y contrastes «no X, sino Y». Diátaxis decide dónde va un texto por lo que está haciendo quien lo abre, y `docs/public/README.md` ya está partido en sus cuatro: tutorial, guía, referencia, explicación. La que más se rompe es la del tutorial, que elige por el lector y funciona siempre: nada de «también podrías» y lo opcional al final. Un how-to que se llena de porqués es explicación disfrazada; el porqué vive en `02-decisiones-de-diseno.md` o en la sección de explicación.

### El changelog

`CHANGELOG.md`, en la raíz. Formato [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), versiones [SemVer](https://semver.org/).

**Se escribe en INGLÉS**, a diferencia de los nueve documentos y de los textos de la app. Es la misma excepción que los mensajes de commit, y acá tiene un motivo mecánico además del de estilo: **el cuerpo de cada publicación cita su tramo tal cual**, así que el idioma del changelog es el idioma del release.
...
- **Quitar la cuarentena es siempre el acto de una persona, jamás de un camino automático.** Hay exactamente dos actos de persona y dos funciones con nombre cerrado: `--uninstall-cleanup` llama a `RemoveBaseQuarantineForUninstall` (solo desde el cableado de `cmd/kanpachid`), y la decisión explícita del usuario llama a `RemoveBaseQuarantineAtUserRequest` por `port.FirewallPort` (solo desde `core/usecase/quarantine.go`, el caso de uso del consentimiento). Un barrido, un arranque, un evento del motor o un `--reset` no pueden quitarla, y los guardianes de `internal/arch/grupobase_test.go` lo sostienen por nombre y por llamador. `--reset` REPONE la cuarentena solo con la decisión del usuario en sí, porque repone lo que el usuario eligió y no lo que el producto prefiere. El reset conserva `last-room.json`: resetear no es olvidar a dónde volver.
...
daemon/     adapter/ transport/ service/   Go, servicio de Windows, elevado
            ...
            API. Ver adapter/firewall/windows/wfp y adapter/firewall/windows/netfw
...
docs/       Los nueve documentos numerados, más este
```
...
- **El estado y las decisiones viven en el DAEMON; las caras solo pintan.** El registro propio, la credencial del seed y **el apodo** son estado del daemon, con sus métodos en el protocolo, y no un fichero que cada cara lea por su cuenta. El apodo estuvo exceptuado, con el argumento de que viaja como parámetro en cada orden: viajar como parámetro y tener dueño son cosas distintas, y confundirlas costó dos ficheros en la misma carpeta y una máquina con dos nombres, la ventana diciendo «Alvaro» y la sala enseñando «AlvaroGDeskt». Sigue viajando como parámetro de crear y entrar; lo que tiene dueño es quién lo recuerda entre sala y sala. **Y hay un motivo más duro que el orden:** en el producto instalado el directorio de datos deja a Users en solo lectura y la ventana corre con el token del usuario de la sesión, así que de los tres procesos el daemon es el único que puede escribir ahí. Una cara que guarda estado en el directorio de datos es una cara cuyo guardado falla callado.
...
- **Dos grupos de firewall, con dueños distintos.** `Kanpachi` es la sala y la escribe el daemon: al arrancar purga lo etiquetado con ese grupo y aplica el estado deseado. `Kanpachi-base` es la cuarentena, y es la DECISIÓN del usuario: el arranque la escribe, antes de purgar y con un método que solo agrega, únicamente cuando la decisión persistida está en sí; con la decisión en no o sin tomar no escribe nada, y el fallo al escribirla dejó de ser fatal. Decir que sí la escribe y decir que no la quita, por la única retirada que el puerto declara. Sigue puesta con el servicio detenido, salvo que una persona diga lo contrario. Solo `daemon/adapter/firewall/windows/netfw/` y `daemon/adapter/firewall/linux/nftpermits/` pueden nombrar el grupo. El desinstalador purga los dos grupos. La comparación de grupo va por igualdad exacta y jamás por prefijo, porque `Kanpachi` es prefijo de `Kanpachi-base`. Lo vigila `internal/arch/grupobase_test.go`.
