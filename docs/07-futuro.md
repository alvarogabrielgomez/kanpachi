...
## 18. Lo que sale del arreglo de la sala muerta, y no entró en 0.6.9

Los cuatro salieron del trabajo del 2026-08-25 y ninguno es un fallo de hoy. Entran juntos en 0.7.0, y ese corte no es de gusto: un arreglo urgente de una caída de treinta y tres horas no puede llevar un refactor en la misma etiqueta, porque si algo sale mal después de desplegar no hay forma de saber cuál de los dos fue.

**El libro de credenciales baja a disco.** Hoy vive en memoria, así que un host que reinicia el daemon y retoma la sala pierde el lazo entre dirección y credencial de quien ya estaba dentro. Eso cuesta dos cosas concretas: no puede expulsar a esa gente hasta que vuelvan a entrar, y la compuerta tiene que seguir mirando la tabla del motor además del libro, que es la mitad que la decisión 43 quiere quitar. **Lo que NO compra, y hay que decirlo antes de escribirlo:** no devuelve a nadie a la sala. El motor es un hijo que muere con el daemon y su lista de credenciales muere con él, así que renovar una credencial de antes del reinicio es error duro por contrato. Compra la dirección estable al volver y poder expulsar.

Va en fichero propio y sellado, y necesita **un contador de generación en el texto claro**. El sello ya cifra y autentica, así que una firma encima no compraría nada: quien pueda falsificarla ya puede leer la llave de identidad. El hueco real es la reversión, porque una copia sellada más vieja de la misma máquina autentica perfecto, y restaurar un libro anterior a una expulsión le devolvería al expulsado su dirección y su ranura pre-autorizada en el oyente que corre como SYSTEM. También necesita su propio párrafo sobre qué guarda: la llave de miembro es pública, pero es estable y enlazable, y guardarla crea un registro durable de quién jugó con quién, cuando el lado invitado ya se niega a conservar su mitad tras una salida deliberada.

**Un solo registro por miembro, con vector de procedencia.** Once almacenes contestan hoy «quién está» y nada los reconcilia. Cinco son fuentes legítimas que saben cosas que ninguna otra sabe; los otros seis son acumulación, y tres de ellos son mapas por dirección bajo el mismo candado, barridos en los mismos sitios y leídos por las mismas funciones. Lo que se busca no es «una sola fuente», que es imposible, sino un solo registro, una sola ruta de actualización y un solo sitio donde mirar. Toca los cinco escritores de `state.Peers` y unas veinte vistas derivadas, que es exactamente por qué no entra en un arreglo urgente.

**El motor espera a que la ruta lleve dirección antes de emitir.** Hoy emite con `PeerConnAdded` y su `peers()` descarta toda ruta sin dirección, así que la relectura que dispara devuelve una lista sin el que acaba de entrar. Arreglarlo en la raíz es releer `list_route` hasta que ese par lleve `ipv4_addr` o venza un plazo corto, y emitir una vez. El vigía de malla lo cubre desde 0.6.9 y lo seguirá cubriendo: mira la tabla ya convergida, así que es la red de seguridad de esto, del descarte por búfer lleno y de cualquier evento futuro que se pierda. Hacen falta los dos, y en ese orden, porque esto viaja en una imagen del motor y el vigía no.

**La exención de loopback en el bloqueo por rango.** Medido el 2026-08-25 en un `netns` limpio: `SlotRoomNet` atrapa el tráfico de loopback dirigido al rango de la sala. No entró en 0.6.9 porque no se encontró un solo consumidor que marque hoy la dirección de la sala desde la propia máquina, y porque la mitad de Windows está sin medir. El disparador es cualquiera de las dos: que aparezca ese consumidor, o que alguien mida WFP. El arreglo sería de una regla, exención de `iif lo` solo en las dos ranuras acotadas por rango, y las acotadas por adaptador no se tocan porque `iif lo` no casa con ellas. Ver la decisión 43.
