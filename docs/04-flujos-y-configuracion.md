...
4. **El log y los datos viajan los dos al lado del bundle**, igual que en `roombundle`, con `--log` y `--data`. Hubo una versión que mandaba solo el log, con el argumento de que `--data` rompía el contrato del marcador: la interfaz deduce su directorio de datos de dónde está ella, que es el temporal. Corrida, el precio resultó ser otro producto: `identity.key` se estrenaba en cada arranque, así que quien abría el `.exe` dos veces era dos equipos distintos, y las extracciones que no se pudieron borrar quedaban en el temporal con la llave dentro. Ahora el daemon le DICE su directorio a la interfaz al lanzarla, por la misma vía que el log.
